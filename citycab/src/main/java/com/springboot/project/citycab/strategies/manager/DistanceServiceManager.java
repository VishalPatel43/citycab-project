...
public class DistanceServiceManager {
    ...
    private final DistanceServiceHereRoutingImpl distanceServiceHereRouting;
    ...
    public double calculateDistance(Point src, Point dest) {
        ...
        Future<Double> osrmFuture = executor
                .submit(
                        () -> distanceServiceOSRM.calculateDistance(src, dest)
                );
        try {
            // Attempt to get the OSRM result within 1 second
            return osrmFuture.get(1, TimeUnit.SECONDS);
        } catch (TimeoutException te) {
            System.out.println("OSRM API call timed out after 1 second. Falling back to Here Routing.");
    ...
    private double calculateWithHereRouting(Point src, Point dest) {
        try {
            return distanceServiceHereRouting.calculateDistance(src, dest);
        } catch (DistanceRestClientServiceException e) {
            System.out.println("Here Routing service failed: " + e.getMessage());
            throw new DistanceRestClientServiceException("Both OSRM and Here Routing services failed.", e);
        }
    }
}
*/
